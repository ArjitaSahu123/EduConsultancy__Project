...
const Pricingcard = () => {
    ...
    const handleBuyNow = (plan) => {
        ...
        setShowForm(true);
        // toast.info(`Selected plan: ${plan.title} (${plan.price})`);
    };
    ...
    return (
        <div className="flex">
            {plans.map((plan, index) => (
                <div
                    key={index}
                    className={`pricingcard flex flex-col gap-10 rounded-3xl ${plan.title === 'Premium' || plan.title === 'Java Core & Advance'
                            ? 'bg-[#060432] text-white'
                            : ''
                        } ${selectedPlan?.title === plan.title && plan.title === 'Free' ? 'border-4 border-green-500' : ''}`} // Highlight the free plan
                >
                    <div className="flex flex-col justify-center items-center gap-5">
                        ...
                        <div>
                            {plan.title === 'Free' ? (
                                <button
                                    className="border-4 border-orange-500 bg-white text-orange-500 font-bold py-2 px-4 rounded-lg"
                                    disabled
                                >
                                    Default Plan
                                </button>
            ...
            {showForm && (
                <div className="fixed inset-0 bg-black bg-opacity-50 flex justify-center items-center">
                    <ProductForm
                        prefilledData={{ course: selectedPlan?.title, amount: selectedPlan?.price }}
                        onClose={closeForm}
                    />
                </div>
            )}
